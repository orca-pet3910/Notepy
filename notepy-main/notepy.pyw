...
previous_workingfile = None

prev_dir = os.getcwd()
os.chdir(script_dir)
os.system("desktop_shortcut.js")
os.chdir(prev_dir)
...
def open_file():
    ...
    if file_path:
        previous_workingfile = file_path
        with open(file_path, mode='r', encoding='utf-8', errors='replace') as file:
            content = file.read()
            text_area.insert(1.0, content)
    if not file_path == "":
        root.title(f"Notepy - {os.path.basename(file_path)} | {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}")
    else:
        pass

def save_file():
    if previous_workingfile == None:
     ...
     if file_path:
        ...
        with open(previous_workingfile, mode='w', encoding='utf-8', errors='replace') as file:
            data = text_area.get(1.0, tk.END)
            file.write(data)
